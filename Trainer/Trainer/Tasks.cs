...
namespace Trainer
{
    public class Tasks
    {
        ...
        public string Make(int WayMeet, int Speed1, int Speed2, int Time, int Way, int Type)
        {
            ...
            s[1] = String.Format("Какое расстояние будет между {0}ами через {1} ч, если известно, что скорость первого {0}а равна {2} км/ч, скорость второго равна {3} км/ч., а расстояние между пунктами равно {4} км", _Entity[n], Time, Speed1, Speed2,Way);
            s[2] = String.Format("Какое расстояние будет от пункта А  до места встречи двух {0}ов, если известно, что скорость первого {0}а равна {1} км/ч, скорость второго равна {2} км/ч, время движения при этом составило {3} ч, а общий путь равен {4} км.", _Entity[n], Speed1, Speed2, Time, Way);
